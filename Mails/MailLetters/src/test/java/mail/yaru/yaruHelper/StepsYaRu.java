...
public class StepsYaRu extends AbstrUser {
	...
	public DraftYaRu writeLetter(){
		...
		draftYaRu=writeLetter.enterRecipient(getLogin()).enterSubject(getTheme()).enterСontent(getContent()).clickCancelButton().saveToDruftButton(writeLetter.driverTo());
		return draftYaRu;
	}
	...
	public PersAccountYaRu sendLetter() {
			  Assert.assertEquals(getTheme(),sendLetter.driverTo().findElement(By.id("compose-subj")).getAttribute("value"));		
			  Assert. assertEquals(getContent(), sendLetter.driverTo().findElement(By.id("compose-send")).getAttribute("value"));		
	...
	Assert.assertEquals(draftYaRu.driverTo().findElement(By.xpath("//div[@class='b-messages__placeholder-item'][1]")).getText(),"В папке «Черновики» нет писем.");
